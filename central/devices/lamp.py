...
class Lamp(Device):
    ...
    def change_colour(self, new_colour: str):
        ...
        if isinstance(new_colour, Colour):
            ...
            self.notificar(f"Cor da lampada '{self.name}' alterada para {Colour(new_colour).name}")
        else:
            self.notificar("Erro: Cor inválida.")

    def pick_colour(self):
        self.notificar(f"A cor atual da lampada é {self._colour.name}")
    ...
    def change_brightness(self, level: int):
        ...
        if 1 <= level <= 100:
            ...
            self.notificar(f"Brilho da lampada '{self.name}' ajustado para {level}%.")
        elif level == 0:
            self.off()
            self.notificar(f"Lampada {self.name} desligada")
        else:
            self.notificar("Erro: Brilho inválido, selecione valor entre 0 e 100.")

    def __init__(self, device_name: str):
        super().__init__(device_name=device_name, device_type="lamp")
        colour = Colour.NEUTRA
        brightness = 100
        self._colour = colour
