...
class Feeder(Device):
    ...
    def refilled(self):
        self.notificar(f"Alimentador '{self.name}' reabastecido")
        self.level = 100 # Assumindo um reabastecimento de 100%

    def on_enter_LIGADO(self):
        self.notificar(f"Alimentador '{self.name}' em modo standby")

    def on_enter_ALIMENTANDO(self):
        self.notificar(f"Alimentador '{self.name}' enchendo o pote de racao")

    def on_enter_VAZIO(self):
        self.notificar(f"Alimentador '{self.name}' precisa ser recarregado!")
